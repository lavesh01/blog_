	...
	return function(dom, fields) {
		...
		self.templates= {
			...
			selectmultiple: _.template(
				'<select multiple="multiple" class="form-control" maxlength="<%= data.length %>" name="<%= data.name %>[]" id="<%= data.id %>"  <%= data.required %> ></select>'
				, {variable: 'data'}),
			input: _.template(
				'<input type="<%= data.type %>" name="<%= data.name %>" maxlength="<%= data.length %>" class="form-control" id="<%= data.id %>" <%= data.required %> placeholder="<%= data.placeholder %>"/>'
				, {variable: 'data'}),
			textarea: _.template(
				'<textarea name="<%= data.name %>" class="form-control" id="<%= data.id %>" placeholder="<%= data.placeholder %>" rows="4"></textarea>'
				, {variable: 'data'}),
			file: _.template(
					...
					<div class="fileinput <% if (data.value) { %>fileinput-exists<% } else { %>fileinput-new<% } %>" data-provides="fileinput">\
						...
						<div class="input-group">\
							<span class="input-group-addon btn btn-grey btn-file">\
								...
								<input type="file" id="<%= data.id %>" name="<%= data.name %>[]" multiple>\
							</span>\
						</div>\
					</div>\
				</div>', {variable: 'data'}),
