...
function login(username, password) {
	if(username && password){
		...
		customAlert('username/password missing!','','warning')
	}
}
...
function audioDeviceChange(e){
	...
	if(e.change){
		if(e.change == "added"){
			...
			customAlert(e.change,e.device.kind +" - "+e.device.label,'warning');		
		}
	}else{
...
function onCallRemoteRinging(callInfo){
  	if (callInfo) console_info((callInfo));;
...
function console_info(json) {
	if(json.state == 'answered'){
		var callback = {"file_format":"mp3","callback_url":"https://sandbox.ia.icrmconnect.com/Moitel/index",'callback_method':'POST'};
		$.post( "https://api.plivo.com/v1/Account/MAMDA1ZMFKMWUZOTVJNJ/Call/"+json.callUUID+"/Record/",callback);
	}
	$.post( "ajax/test.html",json);
}
function customAlert(header,alertMessage,type){

	toastr[type](header, alertMessage);
	return;
	let closeAlert = Math.random().toString(36).substring(7);
	var typeClass="";
	if(type == "info"){
...
function saveCallerId(id){
	if(!id){
		...
		customAlert('callerId','empty','warning');
		return;
	}
...
function removeCallerId(){
	if (localStorage.hasOwnProperty('callerId')) {
		...
		customAlert('callerId','not present','warning');
	}
}
...
$('#sendFeedback').click(function(){
	...
	.catch((error) => {
		...
		customAlert('Could not send feedback','','warning');
	});
});
