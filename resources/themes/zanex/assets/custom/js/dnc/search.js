var adata1 = ajax_call_no_alert("dnc/getLast5Search/",'',true,'JSON','test(data)' );

function test(data)
{ 
  for (var i = data.result.length - 1; i >= 0; i--) {
     console.log(data.result[i].result);
     li(data.result[i].contact,data.result[i].result);
  } 
}
var action = [];
  var aocolumnDefs =  [
...
enable_common_dataTable( 'Dnc/dtable' ,"#edm_dataTable",action,aocolumnDefs);
...
*/
  ...
  var send_data = $.param({"txt_Search_number_ctps":txt_Search_number_ctps , "type":type});
  var adata = ajax_call("dnc/ajax_search/",send_data );
...
$("#upload_prospect_file").click(function (argument) {
   ...
   var url =site_url + "dnc/upload_bulk_check/" ;
