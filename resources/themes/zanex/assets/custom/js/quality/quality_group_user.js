...
enable_common_dataTable( 'Quality/groupuserlist' ,htmldatatable_id,action);
...
(function(window, document, $) {
 ...
 select2_ajax(".select2_campaign", "/Udc/Udc/campaign", "Select Campaign Name");
...
$(document).on('click','.view_user',function(){
  ...
  enable_common_dataTable( 'quality/groupusersListing' ,"#groupusers_data",action,{},"",group_data);
 ...
 $("#ajax_id").val($(this).attr('id').replace("delete_",""));
    Swal.fire({
      ...
      if (result.value) {
        ...
        var adata = ajax_call("Quality/delete_group_users",send_data);
...
$(document).ready(function () {       
  ...
  $('#assign_campaign_togroup').on('click', function () {
   ...
   if(campaign_id=="")
    ...
    var adata = ajax_call("quality/assignCampaignToGroupData",send_data );
...
function draw_table(type="") {
  ...
  if (type=="") {
    dt_test = enable_common_dataTable( 'Quality/groupuserlist' ,htmldatatable_id,action);
