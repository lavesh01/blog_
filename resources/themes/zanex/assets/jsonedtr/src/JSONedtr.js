	...
	JSONedtr.level = function ( node, lvl=0 ) {
		...
		$.each( node , function( key, value ) {
			...
			if( typeof value == 'object' ) {
				...
				output += '<div class="jse--row jse--row--'+type+'" id="jse--row-' + JSONedtr.i + '"><input type="text" class="jse--key jse--'+type+'" data-level="' + lvl + '" value="' + key + '"> : <span class="jse--typeof">(' + type + ')</span>';
				output += JSONedtr.level( value, lvl+1 );
				output += '</div>';
			} else {
				if( typeof value == 'string' )
					value = value.replace(/\"/g,"&quot;");
				output += '<div class="jse--row" id="jse--row-' + JSONedtr.i + '"><input type="text" class="jse--key" data-level="' + lvl + '" value="' + key + '"> : <span class="jse--typeof">(' + typeof value + ')</span><input type="text" class="jse--value" value="' + value + '" data-key="' + key + '"></div>';
			}
		})

		//output += '<div class="jse--row jse--add" data-level="' + lvl + '"></div>';
...
/*
  ...
  $.each( node, function() {
    ...
    if( $(this).hasClass( 'jse--array' ) ) {
      // Remove the index value and just use an empty string as the key
      var selector = '#' + $(this).parent().attr('id') + ' > .jse--row > input';
      result[ '' ] = [JSONedtr.getData( $( selector ) )];
    }
  });
  return result;
}
...
JSONedtr.getData = function (node = $(JSONedtr.outputElement + ' > .jse--row > input')) {
    ...
    function getDataRecursively(node) {
        var obj = {};
        ...
        for (let i = 0; i < node.length; i++) {
            ...
            if ($element.hasClass('jse--value')) {
                obj[$element.data('key')] = $element.val();
            } else if ($element.hasClass('jse--object') || $element.hasClass('jse--array')) {
                const selector = '#' + $element.parent().attr('id') + ' > .jse--row > input';
                const childData = getDataRecursively($(selector));

                if ($element.hasClass('jse--array')) {
                    obj[$element.val()] = [childData._ || childData];
                } else {
                    obj[$element.val()] = childData;
                }
            }
        }
    ...
    const data = getDataRecursively(node);
    result = data;
