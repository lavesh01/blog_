   ...
   if ($query->num_rows() > 0) {
     $env = getenv('CI_ENV');
       foreach ($query->result() as $row) {
         ...
         $this->CI->db->set('next_run_at', 'FROM_UNIXTIME('.$this->calculateNextRun($row).')', false)->where('id', $row->id)->update('bf_cron');
         $output = shell_exec("php /{fullpath}/index.php "$cmd);
         $this->CI->db->set('last_run_at', 'now()', false)->where('id', $row->id)->update('bf_cron');
       }
    } 
  }
}